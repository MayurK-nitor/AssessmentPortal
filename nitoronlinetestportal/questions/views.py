...
def delete_question(request):
    request_body = request.data
    print("request_body_in delete question", request_body)
    if not isinstance(request_body, dict) or not request_body:
        return standard_json_response(message='request body must be non-emtpy object', status_code=status.HTTP_400_BAD_REQUEST)
...
def get_question_details(request, question_id, question_type):
    if question_id:
        if question_type=="1":
            ...
            questions = MultipleChoicesAnswer.objects.get(question_id=question_id)
        ...
        if question_type=="2":
            ...
            program_detail = ProgramTestCase.objects.get(question_id=question_id)
